...
function TierCard({ label, recommended, equipment, allEquipment, onSelect, laborCost, taxRate }) {
  ...
  return (
    <div className={`rounded-xl border-2 p-4 space-y-3 ${recommended ? 'border-amber-400 bg-amber-50/40' : 'border-gray-200 bg-white'}`}>
            ...
            <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-white border border-gray-200 rounded-lg shadow-xl overflow-y-auto max-h-64">
              ...
              {allEquipment.map(item => (
                <button key={item.id} type="button" onClick={() => { onSelect(item); setOpen(false); }}
                  className={`w-full flex items-center justify-between px-3 py-2.5 text-sm hover:bg-sky-50 transition-colors text-left ${equipment?.id === item.id ? 'bg-sky-50 font-semibold' : ''}`}>
      ...
      {equipment && (
        <div className="bg-gray-50 rounded-lg px-3 py-2.5 space-y-1 text-sm">
          <div className="flex justify-between text-gray-600"><span>Equipment</span><span>{formatCurrency(equipPrice)}</span></div>
          <div className="flex justify-between text-gray-600"><span>Labor</span><span>{formatCurrency(labor)}</span></div>
          <div className="flex justify-between text-gray-600"><span>Tax ({tax}%)</span><span>{formatCurrency(taxAmount)}</span></div>
...
export default function EditBidModal({ isOpen, onClose, bid, jobMaterials, onSaveSuccess, defaultSettings }) {
  ...
  // Fetch the parent job — labor cost source of truth lives here
  const { data: parentJob } = useQuery({
    queryKey: ['job-for-bid', bid?.jobId],
    queryFn: () => base44.entities.Job.filter({ id: bid.jobId }).then(r => r[0] ?? null),
    enabled: isOpen && !!bid?.jobId,
    staleTime: 0,
  });
  ...
  // Populate all scalar fields when modal opens.
  // Labor cost comes from the parent job — job is the single source of truth.
  useEffect(() => {
  ...
  // Load labor cost from parent job once it arrives (job is source of truth)
  useEffect(() => {
    if (parentJob != null) {
      setLaborCost(String(parentJob.laborCost ?? 0));
    }
  }, [parentJob]);

  // Pre-select tier equipment once equipment list loads
  useEffect(() => {
  ...
  const buildTierData = (key) => {
    ...
    return {
      equipmentId: item.id, equipmentName: item.name, equipmentPrice: item.defaultCost,
      subtotal: item.defaultCost, labor, tax: taxAmt,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const finFeeAmt = financingEnabled && parseFloat(financingFeePercent) > 0
        ? preTaxBase * (parseFloat(financingFeePercent) / 100) : 0;
      ...
      // Write labor cost back to the job — it's the single source of truth
      if (bid.jobId) {
        const totalMaterialCost = (jobMaterials || []).reduce((sum, m) => sum + (m.totalCost || 0), 0);
        await base44.entities.Job.update(bid.jobId, {
          laborCost: labor,
          totalPrice: totalMaterialCost + labor,
        });
        queryClient.invalidateQueries({ queryKey: ['job', bid.jobId] });
        queryClient.invalidateQueries({ queryKey: ['jobs'] });
        queryClient.invalidateQueries({ queryKey: ['job-for-bid', bid.jobId] });
      }

      // Save all other fields to the bid (labor cached here for PDF rendering)
      await base44.entities.Bid.update(bid.id, {
        documentType, estimateType, tiers: tiersData,
        materialSubtotal: materialSub, laborCost: labor,
        taxRate: tax, taxAmount, totalAmount,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={o => { if (!o) { setSaveError(null); setIsSaving(false); onClose(); } }}>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden mt-1">
              <button type="button" onClick={() => setEstimateType('standard')}
                className={`flex-1 py-2 text-sm font-medium transition-colors ${estimateType === 'standard' ? 'bg-[#071a35] text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                Standard
              </button>
              <button type="button" onClick={() => setEstimateType('tiered')}
                className={`flex-1 py-2 text-sm font-medium transition-colors ${estimateType === 'tiered' ? 'bg-[#071a35] text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                Good / Better / Best
              </button>
            </div>
          </div>
          ...
          <div className="bg-muted/30 rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mb-1">Synced with job record</p>
                <Input type="number" step="1" min="0" value={laborCost} onChange={e => setLaborCost(e.target.value)} />
              </div>
              <div>
          ...
          {estimateType === 'tiered' && (
            <div className="space-y-3">
              ...
              <TierCard label="Good" equipment={tierEquipment.good} allEquipment={allEquipment} onSelect={item => setTierItem('good', item)} laborCost={laborCost} taxRate={taxRate} />
              <TierCard label="Better" recommended equipment={tierEquipment.better} allEquipment={allEquipment} onSelect={item => setTierItem('better', item)} laborCost={laborCost} taxRate={taxRate} />
              <TierCard label="Best" equipment={tierEquipment.best} allEquipment={allEquipment} onSelect={item => setTierItem('best', item)} laborCost={laborCost} taxRate={taxRate} />
            </div>
          )}

          <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-4 space-y-3">
            <div className="flex items-center gap-3">
              <input type="checkbox" id="editFinancingEnabled" checked={financingEnabled}
                onChange={e => { setFinancingEnabled(e.target.checked); if (!e.target.checked) { setFinancingProvider(''); setFinancingFeePercent(''); } }}
            ...
            {financingEnabled && (
              <div className="space-y-2 pt-1">
                ...
                {financeVendors.length > 0 ? (
                  <Select value={financingProvider} onValueChange={val => { setFinancingProvider(val); const v = financeVendors.find(f => f.name === val); if (v) setFinancingFeePercent(String(v.feePercent)); }}>
                    <SelectTrigger className="mt-1"><SelectValue placeholder="Select financing vendor..." /></SelectTrigger>
                    <SelectContent>{financeVendors.map((v, i) => <SelectItem key={i} value={v.name}>{v.name} ({v.feePercent}% fee)</SelectItem>)}</SelectContent>
                  </Select>
                ) : (
                ...
                {financingFeePercent && parseFloat(financingFeePercent) > 0 && (
                  <p className="text-xs text-[#2E9CCA] font-medium">{financingFeePercent}% financing fee will be added to the total (pre-tax)</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="bg-muted/30 rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div><Label>Deposit (%)</Label><Input type="number" step="1" min="0" max="100" value={depositPercent} onChange={e => setDepositPercent(e.target.value)} /></div>
              <div><Label>Progress (%)</Label><Input type="number" step="1" min="0" max="100" value={progressPercent} onChange={e => setProgressPercent(e.target.value)} /></div>
              <div><Label>Balance (%)</Label><Input type="number" step="1" min="0" max="100" value={balancePercent} onChange={e => setBalancePercent(e.target.value)} /></div>
            </div>
            ...
            <div><Label>Progress Milestone</Label><Input value={progressMilestone} onChange={e => setProgressMilestone(e.target.value)} placeholder="e.g., equipment delivery" /></div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleSave} disabled={isSaving} className="bg-secondary hover:bg-secondary/90 text-secondary-foreground">
